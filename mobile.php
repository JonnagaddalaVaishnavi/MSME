...
<div class="container">
    <div class="products"><br><br>
        <div class="box-container">
            <?php
            $unique_images = array();
            $select_product = mysqli_query($conn, "SELECT DISTINCT * FROM `mobiles`") or die(mysqli_error($conn));
            if (mysqli_num_rows($select_product) > 0) {
                while ($fetch_product = mysqli_fetch_assoc($select_product)) {
                    if (!in_array($fetch_product['image'], $unique_images)) {
                        $unique_images[] = $fetch_product['image'];
            ?>
                        <div class="box">
